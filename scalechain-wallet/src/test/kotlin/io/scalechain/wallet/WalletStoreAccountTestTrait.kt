  ...
  fun testWalletStoreAccount(store: WalletStore, db : KeyValueDatabase) : FlatSpec {
    return object : FlatSpec() {
      init {
        ...
        "putReceivingAddress".config(ignored=true) should "put a receiving address for an account" {
        ...
        "getOutputOwnerships".config(ignored=true) should "NOT get output ownerships of another account." {
          // No need to test this case, as it was tested in the following test.
          // "getOutputOwnerships" should "get output ownerships that an account has if Some(account) is passed for the parameter."
        }
