...
class SendRawTransactionSpec : FlatSpec with BeforeAndAfterEach with APITestSuite {
  ...
  "SendRawTransaction" should "send a serialized transaction. (with allowHighFees argument true)" {
    val response = invoke(SendRawTransaction, listOf(SERIALIZED_TRANSACTION, JsBoolean(true)))
    val result = response.right.get.get.asInstanceOf<StringResult>
  ...
  "SendRawTransaction" should "send a serialized transaction. (with allowHighFees argument false)" {
    val response = invoke(SendRawTransaction, listOf(SERIALIZED_TRANSACTION, JsBoolean(false)))
    val result = response.right.get.get.asInstanceOf<StringResult>
