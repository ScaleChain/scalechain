...
object GetBlocksMessageHandler {
  ...
  def handle(context: MessageHandlerContext, getBlocks: GetBlocks): Unit = {
    try {
      ...
      // Each peer keeps a list of blocks requested using getblocks not to request the same block over and over.
      // clear blocks requested so that we can send getblocks message.
      context.peer.clearBlocksRequested()
    }
  }
}