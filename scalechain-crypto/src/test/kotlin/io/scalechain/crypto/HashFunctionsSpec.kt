...
class HashFunctionsSpec : FlatSpec(), Matchers {
  ...
  init {
    ...
    "sha256(ByteBuf)" should "return a correct hash value" {
      // expected value calculated from the following site :
      // http://hash.online-convert.com/sha256-generator
      val buffer : ByteBuf = Unpooled.wrappedBuffer("hello world".toByteArray())

      HashFunctions.sha256(buffer) shouldBe SHA256( Bytes(bytes("b94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9")) )
    }

    "sha256(ByteArray)" should "return a correct hash value" {
      // expected value calculated from the following site :
      // http://hash.online-convert.com/sha256-generator
    ...
    "sha256(ByteArray, offset, length)" should "return a correct hash value" {
      // expected value calculated from the following site :
      // http://hash.online-convert.com/sha256-generator
...
//      println("${hex(HashFunctions.hash160("hello world".toByteArray()).value.array)}")
    ...
    "hash256(ByteBuf)" should "return a correct hash value" {
      val buffer : ByteBuf = Unpooled.wrappedBuffer("hello world".toByteArray())
      HashFunctions.hash256(buffer) shouldBe Hash256( Bytes(bytes("bc62d4b80d9e36da29c16c5d4d9f11731f36052c72401a76c23c0fb5a9b74423")) )
    }

    "hash256(ByteArray)" should "return a correct hash value" {
//      println("${hex(HashFunctions.hash256("hello world".toByteArray()).value.array)}")
    ...
    "hash256(ByteArray, offset, length)" should "return a correct hash value" {
