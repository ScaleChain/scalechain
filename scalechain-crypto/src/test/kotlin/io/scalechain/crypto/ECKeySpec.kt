...
class ECKeySpec : FlatSpec(), Matchers {
  ...
  init {
    "ECKey.ECDSASignature.isEncodingCanonical" should "validate a correct signature format" {
      val signature : ByteArray = HexUtil.bytes("304402203f16c6f40162ab686621ef3000b04e75418a0c0cb2d8aebeac894ae360ac1e780220ddc15ecdfc3507ac48e1681a33eb60996631bf6bf5bc0a0682c4db743ce7ca2b01")

      ECKey.ECDSASignature.isEncodingCanonical(signature) shouldBe true
    }

    "sign" should "correctly sign input data" {
      val ZERO_HASH : ByteArray = ListExt.fill(16, 0.toByte()).toByteArray()
      // Test that we can construct an ECKey from a private key (deriving the public from the private), then signing
      // a message with it.
      val privkey : BigInteger = BigInteger(1, HexUtil.bytes("180cb41c7c600be951b5d3d0a7334acc7506173875834f7a6c4c786a28fcbb19"));
      val publicKey : ByteArray = ECKey.publicKeyFromPrivate(privkey, true)
